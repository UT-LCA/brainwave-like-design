...
assign comparator_output_final = comparator_output_0_stage_${num_comparison_stages};
...
%for i in range(0,num_tiles,2):
    ...
    generate
        for(i=1; i<=`NUM_LDPES; i=i+1) begin
           comparator #(.DWIDTH(`BFLOAT_EXP)) comparator_units_initial_${int(i/2)} (
              .a(inp${i}[i*`BFLOAT_EXP-1:(i-1)*`BFLOAT_EXP]),
              .b(inp${i+1}[i*`BFLOAT_EXP-1:(i-1)*`BFLOAT_EXP]),
              .clk(clk),
              .reset(reset),
			  .start(start),
              .out(comparator_output_${int(i/2)}_stage_1[i*(`BFLOAT_EXP)-1:(i-1)*(`BFLOAT_EXP)])
            );
        end
    endgenerate
%endfor
...
% for k in range(num_tiles>>(i+1)):
    ...
    generate
        for(i=1; i<=`NUM_LDPES; i=i+1) begin
           comparator #(.DWIDTH(`BFLOAT_EXP)) comparator_units_${k}_stage_${i} (
              .a(comparator_output_${2*(k)}_stage_${i}[i*(`BFLOAT_EXP)-1:(i-1)*(`BFLOAT_EXP)]),
              .b(comparator_output_${(2*k)+1}_stage_${i}[i*(`BFLOAT_EXP)-1:(i-1)*(`BFLOAT_EXP)]),
              .clk(clk),
              .reset(reset),
			  .start(start),
              .out(comparator_output_${k}_stage_${i+1}[i*(`BFLOAT_EXP)-1:(i-1)*(`BFLOAT_EXP)])
            );
        end
    endgenerate
%endfor
...
assign comparator_output_final[${i+1}*`BFLOAT_EXP-1:${i}*`BFLOAT_EXP] = comparator_output_0_stage_${num_reduction_stages}[${i+1}*(`BFLOAT_EXP)-1:${i}*(`BFLOAT_EXP)];
% endfor 
endmodule
