<%!
    ...
    brams_per_mrf = int(num_dsp_per_ldpe*mac_per_dsp*in_precision/mat_bram_dwidth)
%>
...
% for i in range(1,num_ldpes+1):
        msfp11_to_fp16  msfp_to_fp_converter_${i}(
            .clk(clk),
            .reset(reset[${i}-1]),
...
% for i in range(1,num_dsp_per_ldpe+1):
            fp16_to_msfp11 fp_converter_ax_${i}(.rst(reset),.start(start),.a(ax[${i}*`FLOAT_DWIDTH-1:(${i}-1)*`FLOAT_DWIDTH]),.b(ax_in_sub_ldpe[${i}*`BFLOAT_DWIDTH-1:(${i}-1)*`BFLOAT_DWIDTH]),.out_data_available(out_data_available_ax[${i}-1]),.clk(clk));
% endfor

    wire[`DSPS_PER_LDPE-1:0] out_data_available_ay;

% for i in range(1,num_dsp_per_ldpe+1):
            fp16_to_msfp11 fp_converter_ay_${i}(.rst(reset),.start(start),.a(ay[${i}*`FLOAT_DWIDTH-1:(${i}-1)*`FLOAT_DWIDTH]),.b(ay_in_sub_ldpe[${i}*`BFLOAT_DWIDTH-1:(${i}-1)*`BFLOAT_DWIDTH]),.out_data_available(out_data_available_ay[${i}-1]),.clk(clk));
% endfor

    wire[`DSPS_PER_LDPE-1:0] out_data_available_bx;

% for i in range(1,num_dsp_per_ldpe+1):
            fp16_to_msfp11 fp_converter_bx_${i}(.rst(reset),.start(start),.a(bx[${i}*`FLOAT_DWIDTH-1:(${i}-1)*`FLOAT_DWIDTH]),.b(bx_in_sub_ldpe[${i}*`BFLOAT_DWIDTH-1:(${i}-1)*`BFLOAT_DWIDTH]),.out_data_available(out_data_available_bx[${i}-1]),.clk(clk));
% endfor

    wire[`DSPS_PER_LDPE-1:0] out_data_available_by;

% for i in range(1,num_dsp_per_ldpe+1):
            fp16_to_msfp11 fp_converter_by_${i}(.rst(reset),.start(start),.a(by[${i}*`FLOAT_DWIDTH-1:(${i}-1)*`FLOAT_DWIDTH]),.b(by_in_sub_ldpe[${i}*`BFLOAT_DWIDTH-1:(${i}-1)*`BFLOAT_DWIDTH]),.out_data_available(out_data_available_by[${i}-1]),.clk(clk));
% endfor
...
% for i in range(1,brams_per_mrf+1):
        sp_ram # (
...
% for j in range(1,num_ldpes+1):
           myadder #(.INPUT_WIDTH(`DSP_USED_OUTPUT_WIDTH),.OUTPUT_WIDTH(`DSP_USED_OUTPUT_WIDTH)) adder_units_initial_${int(i/2)}_${j} (
              .a(inp${i}[${j}*`DSP_USED_OUTPUT_WIDTH-1:(${j}-1)*`DSP_USED_OUTPUT_WIDTH]),
              .b(inp${i+1}[${j}*`DSP_USED_OUTPUT_WIDTH-1:(${j}-1)*`DSP_USED_OUTPUT_WIDTH]),
              .clk(clk),
              .reset(reset_reduction_mvm[${j}-1]),
              .start(start[${j}-1]),
              .out_data_available(out_data_available_${int(i/2)}_stage_1[${j}-1]),
              .sum(reduction_output_${int(i/2)}_stage_1[${j}*`DSP_USED_OUTPUT_WIDTH-1:(${j}-1)*`DSP_USED_OUTPUT_WIDTH])
            );
% endfor
% endfor
...
% for j in range(1,num_ldpes+1):
           myadder #(.INPUT_WIDTH(`DSP_USED_OUTPUT_WIDTH),.OUTPUT_WIDTH(`DSP_USED_OUTPUT_WIDTH)) adder_units_${k}_stage_${i}_${j} (
              .a(reduction_output_${2*(k)}_stage_${i}[${j}*(`DSP_USED_OUTPUT_WIDTH)-1:(${j}-1)*(`DSP_USED_OUTPUT_WIDTH)]),
              .b(reduction_output_${(2*k)+1}_stage_${i}[${j}*(`DSP_USED_OUTPUT_WIDTH)-1:(${j}-1)*(`DSP_USED_OUTPUT_WIDTH)]),
              .clk(clk),
              .reset(reset_reduction_mvm[${j}-1]),
              .start(out_data_available_0_stage_${i}[${j}-1]),
              .out_data_available(out_data_available_${k}_stage_${i+1}[${j}-1]),
              .sum(reduction_output_${k}_stage_${i+1}[${j}*(`DSP_USED_OUTPUT_WIDTH)-1:(${j}-1)*(`DSP_USED_OUTPUT_WIDTH)])
            );
%endfor
%endfor
%endfor
