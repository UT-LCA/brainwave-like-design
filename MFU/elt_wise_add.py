...
for i in range(0,design_size):
    a = '''reg [(`DWIDTH)-1:0] x_{i}; 
    reg [(`DWIDTH)-1:0] y_{i};
    
    add a{i}(.p(out_data[({j}*`DWIDTH)-1:({i}*`DWIDTH)]),.x(x_{i}),.y(y_{i}));
    '''.format(i=i,j=i+1)
    f.write(a)

a = ''' reg[`LOG_ADD_LATENCY-1:0] state;
