...
`define SIGN 1
`define FLOAT_DWIDTH (`SIGN+`EXPONENT+`MANTISSA)
...
module exponent_comparator_tree #(parameter DWIDTH = `INPUT_WIDTH) (
    input[DWIDTH-1:0] inp0,
    input[DWIDTH-1:0] inp1,
    input[DWIDTH-1:0] inp2,
    input[DWIDTH-1:0] inp3,
    output reg[DWIDTH-1:0] result_final_stage,
    
    //CONTROL SIGNALS
    input clk,
    input reset
);
    wire[DWIDTH-1:0] comparator_output_final;
 
    always @(*) begin
        if (reset) begin
          result_final_stage <= 0;
        end
        else begin
    ...
    wire[(DWIDTH)*`NUM_LDPES-1:0] comparator_output_0_stage_1;
    generate
        for(i=1; i<=`NUM_LDPES; i=i+1) begin
           comparator #(.DWIDTH(DWIDTH)) comparator_units_initial_0 (
              .a(inp0[i*DWIDTH-1:(i-1)*DWIDTH]),
              .b(inp1[i*DWIDTH-1:(i-1)*DWIDTH]),
              .clk(clk),
              .reset(reset),
              .out(comparator_output_0_stage_1[i*(DWIDTH+1)-1:(i-1)*(DWIDTH+1)])
            );
        end
    endgenerate
    wire[(DWIDTH)*`NUM_LDPES-1:0] comparator_output_1_stage_1;
    generate
        for(i=1; i<=`NUM_LDPES; i=i+1) begin
           comparator #(.DWIDTH(DWIDTH)) comparator_units_initial_1 (
              .a(inp2[i*DWIDTH-1:(i-1)*DWIDTH]),
              .b(inp3[i*DWIDTH-1:(i-1)*DWIDTH]),
              .clk(clk),
              .reset(reset),
              .out(comparator_output_1_stage_1[i*(DWIDTH+1)-1:(i-1)*(DWIDTH+1)])
            );
        end
    endgenerate

    wire[(DWIDTH)*`NUM_LDPES-1:0] comparator_output_0_stage_2;
    generate
        for(i=1; i<=`NUM_LDPES; i=i+1) begin
           comparator #(.DWIDTH(DWIDTH)) comparator_units_0_stage_1 (
              .a(comparator_output_0_stage_1[i*(DWIDTH)-1:(i-1)*(DWIDTH)]),
              .b(comparator_output_1_stage_1[i*(DWIDTH)-1:(i-1)*(DWIDTH)]),
              .clk(clk),
              .reset(reset),
              .out(comparator_output_0_stage_2[i*(DWIDTH)-1:(i-1)*(DWIDTH)])
            );
        end
    endgenerate

assign comparator_output_final[1*DWIDTH-1:0*DWIDTH] = comparator_output_0_stage_2[1*(DWIDTH)-1:0*(DWIDTH)];
endmodule

module comparator #(parameter DWIDTH=`INPUT_WIDTH) (
    input[DWIDTH-1:0] a,
    input[DWIDTH-1:0] b,
    input reset,
    input clk,
