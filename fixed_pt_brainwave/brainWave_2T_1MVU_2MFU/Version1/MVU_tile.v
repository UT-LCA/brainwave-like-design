...
`define NUM_TILE 2
/*
module MVU( input clk,
    input start,
    input reset,
    input done,
    input [`VRF_AWIDTH-1:0] vrf_rw_addr,
    ...
    wire [`VRF_DWIDTH-1:0] vec_tile0;
    //wire [`VRF_DWIDTH-1:0] vec_tile1;
   // wire [`VRF_DWIDTH-1:0] vec_tile2;
    
    wire [`MRF_DWIDTH-1:0] vrf_data_out0;
  //  wire [`MRF_DWIDTH-1:0] vrf_data_out1;
   // wire [`MRF_DWIDTH-1:0] vrf_data_out2;

    wire [`MRF_DWIDTH-1:0] mrf_in_tile0;
   // wire [`MRF_DWIDTH-1:0] mrf_in_tile1;
   // wire [`MRF_DWIDTH-1:0] mrf_in_tile2;

    wire [`ORF_DWIDTH*`NUM_LDPES-1:0] result0;
   // wire [`ORF_DWIDTH*`NUM_LDPES-1:0] result1;
   // wire [`ORF_DWIDTH*`NUM_LDPES-1:0] result2;

    assign vec_tile0 = vec[`VRF_DWIDTH-1:0];
  //  assign vec_tile1 = vec[2*`VRF_DWIDTH-1:`VRF_DWIDTH];
  //  assign vec_tile2 = vec[3*`VRF_DWIDTH-1:2*`VRF_DWIDTH];

    assign vrf_data_out[`VRF_DWIDTH-1:0] = vrf_data_out0;
 //   assign vrf_data_out[2*`VRF_DWIDTH-1:`VRF_DWIDTH] = vrf_data_out1;
 //   assign vrf_data_out[3*`VRF_DWIDTH-1:2*`VRF_DWIDTH] = vrf_data_out2;

    assign mrf_in_tile0 = mrf_in[`MRF_DWIDTH-1:0];
  //  assign mrf_in_tile1 = mrf_in[2*`VRF_DWIDTH-1:`VRF_DWIDTH];
 //   assign mrf_in_tile2 = mrf_in[3*`VRF_DWIDTH-1:2*`VRF_DWIDTH];
    ...
    MVU_tile tile0(
        ...
        .reset(reset), 
        .done(done), 
        .vrf_wr_enable(vrf_wr_enable), 
        .vrf_wr_addr(vrf_wr_addr), 
        .mrf_we(mrf_we), 
        .mrf_addr(mrf_addr),
        .vec(vec_tile0),
         
        .vrf_data_out(vrf_data_out0), 
        .mrf_in(mrf_in_tile0), 
        .result(result), 
        .vrf_readn_enable(vrf_readn_enable),
        .result_addr(result_addr));
    //MVU_tile tile1(.clk(clk), .start(start), .rst(rst), .done(done), .vec_we(vec_we), .vrf_wr_addr(vrf_wr_addr), .mrf_we(mrf_we), .mrf_addr(mrf_addr),
    .vec(vec_tile1), vrf_data_out(vrf_data_out1), .mrf_in(mrf_in_tile1), .result(result1));

    MVU_tile tile2(.clk(clk), .start(start), .rst(rst), .done(done), .vec_we(vec_we), .vrf_wr_addr(vrf_wr_addr), .mrf_we(mrf_we), .mrf_addr(mrf_addr),
   .vec(vec_tile2), vrf_data_out(vrf_data_out2), .mrf_in(mrf_in_tile2), .result(result2));

    reduction_unit reduction_unit0(.result0(result0),.result1(result1),.result2(result2),.result(result));
endmodule
*/
